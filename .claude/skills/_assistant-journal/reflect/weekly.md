# Journal: Reflect Weekly

Compare planned weekly reflection to actual insights and return findings to Ada.

## Context to Load

1. Read own template from `vault/00_Brain/Systemic/Templates/Assistants/journal/weekly.md`
2. Read plan output: `vault/00_Brain/Synthetic/Assistants/journal/{date}-plan-weekly.md`
3. Read capture note `vault/00_Brain/Captive/Week.md` — find `## Reflections` and `## Insights to Extract` sections by H2 header
4. Read `vault/00_Brain/Semantic/Assistants/journal/memory.md` if exists

## Process

1. Extract planned reflection themes from plan output (what was intended)
...
### Variance
- Planned: [Summary of reflection themes from plan]
...
After coaching, write to `vault/00_Brain/Synthetic/Assistants/journal/{date}-reflect-weekly.md`:

```markdown
# Journal: reflect(weekly) {date}
